clear all;
json_str = fileread('data/java_testgen/coverage/tg_javaConvertedCoverageReport.json');
data = jsondecode(json_str);
image_base_path = 'images/java_testgen/coverage/';
...
build_stacked_bar_chart(data, [x1, x2], 'southeast', 'Java Test Coverage', strcat(image_base_path,'bar_charts/','java_cov_all.png'), 0.9, '%', labels, {'ChatGPT', 'Copilot'});
...
for i = 1:numel(bar_data_full)
    ...
    n = size(data, 1);
    x = 1:n; 
    build_stacked_bar_chart(data, x, 'northeast', strcat('Java', bar_titles_full{i}), strcat(image_base_path,'bar_charts/',bar_image_paths_full{i}), bar_width, '%', bar_ticklabels(i), {});
end
...
n = size(data, 1);
x = 1:n;  
build_stacked_bar_chart(data, x, 'northeast','Java Test Coverage', strcat(image_base_path,'bar_charts/','cov_bar.png'), bar_width, '%', bar_ticklabels, {});
