clear all;
json_str = fileread('data/python/unitTests/pythonUnitTestReport.json');
data = jsondecode(json_str);
image_base_path = 'images/python_12_algorithms/correctness/';
...
bar_image_paths_abs = {
    'python_chatgpt_code_correctness_count.png'; 'python_copilot_code_correctness_count.png'; 
};
...
absolute = 1;
% bar chart split by ai model - absolute numbers
for i = 1:length(bar_data_absolute)
    data = bar_data_absolute(i, :);
    n = size(data, 1); % number of bars
    x = 1:n; % position of the bars
    build_stacked_bar_chart(data, x, 'northeast', y_lim, strcat('Python', bar_titles{i}), strcat(image_base_path, bar_image_paths_abs{i}), bar_width, 'Generation count', xlabels(i), absolute);
end
...
x1 = 1:n;
x2 = (n + 2):(n * 2 + 1);  
build_stacked_bar_chart(bar_data_absolute, [x1, x2], 'northeast', y_lim, 'Python Code Correctness', strcat(image_base_path,'python_code_correctness_count.png'), bar_width, 'Generation count', xlabels, absolute);

% PERCENTAGES
bar_image_paths = {
    'python_chatgpt_code_correctness.png'; 'python_copilot_code_correctness.png'; 
};
absolute = 0;
y_lim = 109;
...
x1 = 1:n;
x2 = (n + 2):(n * 2 + 1);  
build_stacked_bar_chart(bar_data_perc, [x1, x2], 'northeast', y_lim, 'Python Code Correctness', strcat(image_base_path,'python_code_correctness.png'), bar_width, '%', xlabels, absolute);
