clear all;
lines_str = fileread('data/java/lineCount/javaAlgorithmsLineCount.json');
lines = jsondecode(lines_str);
image_base_path = 'images/java_12_algorithms/quality/';
...
chatgpt_lines = [lines.chatGPT.BreadthFirstSearch;
                 ...
                 lines.chatGPT.Kruskal;
                 lines.chatGPT.BellmanFord;
                 lines.chatGPT.DepthFirstSearch;
                 lines.chatGPT.Dijkstra;
                 lines.chatGPT.EgyptianFractions;
                 lines.chatGPT.FloydWarshall];
...
copilot_lines = [lines.copilot.BreadthFirstSearch;
                 ...
                 lines.copilot.Kruskal;
                 lines.copilot.BellmanFord;
                 lines.copilot.DepthFirstSearch;
                 lines.copilot.Dijkstra;
                 lines.copilot.EgyptianFractions;
                 lines.copilot.FloydWarshall];

chatGptTotalLines = sum(chatgpt_lines);
copilotTotalLines = sum(copilot_lines);

% Count number of errors
json_str = fileread('data/java/checkstyle/convertedCheckstyleReport.json');
...
chatgpt_data = [
                ...
                data.ChatGPT.BellmanFord;
                data.ChatGPT.DepthFirstSearch;
                data.ChatGPT.Dijkstra;
                data.ChatGPT.EgyptianFractions;
                data.ChatGPT.FloydWarshall;
                data.ChatGPT.Knapsack;
                data.ChatGPT.Kruskal;
...
copilot_data = [
                ...
                data.Copilot.BellmanFord;
                data.Copilot.DepthFirstSearch;
                data.Copilot.Dijkstra;
                data.Copilot.EgyptianFractions;
                data.Copilot.FloydWarshall;
                data.Copilot.Knapsack;
                data.Copilot.Kruskal;
...
bar_image_paths = {
                   'java_chatgpt_code_quality.png'; 'java_copilot_code_quality.png';
                   };
...
hold off;
if SAVEPNG
    saveas(gcf, strcat(image_base_path, 'java_code_quality_count.png'));
end
...
x1 = 1:n;
x2 = (n + 2):(n * 2 + 1);
build_stacked_bar_chart(bar_data_perc, [x1, x2], 'northeast', y_lim, 'Java Code Quality', strcat(image_base_path, 'java_code_quality.png'), bar_width, '%', xlabels, absolute);
...
ylabel('Count');
title('Java Copilot Quality Errors');
...
box off;
if SAVEPNG
    saveas(gcf, strcat(image_base_path, 'java_copilot_quality_errors_count.png'));
end
...
xlabel('Number of quality errors');
ylabel('Count');
title('Java ChatGPT Quality Errors');
...
box off;
if SAVEPNG
    saveas(gcf, strcat(image_base_path, 'java_chatgpt_quality_errors_count.png'));
end
...
box off;
if SAVEPNG
    saveas(gcf, strcat(image_base_path, 'java_copilot_quality_errors.png'));
end
...
box off;
if SAVEPNG
    saveas(gcf, strcat(image_base_path, 'java_chatgpt_quality_errors.png'));
end
...
xlabel('Number of quality errors');
ylabel('%', 'Rotation', 0);
title('Java Quality Errors Comparison (first 5 entries)');
...
if SAVEPNG
    saveas(gcf, strcat(image_base_path, 'java_quality_errors_comparison.png'));
end
