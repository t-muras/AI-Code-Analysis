clear all;
json_str = fileread('data/python_testgen/levenshtein/tg_pyLevenshteinReport.json');
data = jsondecode(json_str);
json_str = fileread('data/python_testgen/levenshtein/tg_pyCharCounts.json');
dataChars = jsondecode(json_str);
image_base_path = 'images/python_testgen/levenshtein/';
...
build_stacked_bar_chart(data, [x1, x2], 'southeast', 'Python Modified Test Code', strcat(image_base_path,'bar_charts/','python_modCode_all.png'), 0.9, '%', labels, {'ChatGPT', 'Copilot'});
...
for i = 1:numel(bar_data_full)
    ...
    n = size(data, 1);
    x = 1:n; 
    build_stacked_bar_chart(data, x, 'southeast', strcat('Python', bar_titles_full{i}), strcat(image_base_path,'bar_charts/',bar_image_paths_full{i}), bar_width, '%', bar_xlabels_full(i), {});
end
...
n = size(data, 1);
x = 1:n; 
build_stacked_bar_chart(data, x, 'southeast','Python Modified Test Code', strcat(image_base_path,'bar_charts/','modCode_bar.png'), bar_width, '%', bar_xlabels_full, {});
