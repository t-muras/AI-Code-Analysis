clear all;
json_str = fileread('data/python_testgen/unitTests/tg_pyUnitTestReport.json');
data = jsondecode(json_str);
image_base_path = 'images/python_testgen/correctness/';
...
absolute = 1;
% coverage bar chart split by approach and ai model - absolute numbers
for i = 1:3:length(bar_data_absolute)
    ...
    build_stacked_bar_chart(data, x, 'northeast', y_lim, strcat('Python', bar_titles{i}), strcat(image_base_path, bar_image_paths_abs{i}), bar_width, 'Generation count', xlabels, {}, absolute);
end
...
build_stacked_bar_chart(bar_data_absolute, [x1, x2], 'north', y_lim, 'Python Test Correctness', strcat(image_base_path,'correctness_bar_split_abs.png'), 0.9, 'Generation count', labels, {'ChatGPT', 'Copilot'}, absolute);
...
absolute = 0;
y_lim = 109;
% coverage bar chart split by approach and ai model - percentages
for i = 1:3:length(bar_data_perc)
    data = bar_data_perc(i:i+2, :);
    n = size(data, 1); % number of bars
    x = 1:n; % position of the bars
    xlabels = {'PromptOnly', 'BookExampleCode', 'AIGenerated'};
    build_stacked_bar_chart(data, x, 'northeast', y_lim, strcat('Python', bar_titles{i}), strcat(image_base_path, bar_image_paths{i}), bar_width, '%', xlabels, {}, absolute);
end
...
x2 = (n + 2):(n * 2 + 1); 
labels = {'PromptOnly', 'BookExampleCode', 'AIGenerated', 'PromptOnly', 'BookExampleCode', 'AIGenerated'}; 
build_stacked_bar_chart(bar_data_perc, [x1, x2], 'north', y_lim, 'Python Test Correctness', strcat(image_base_path,'correctness_bar_split.png'), 0.9, '%', labels, {'ChatGPT', 'Copilot'}, absolute);
...
for i = 1:numel(bar_data_full)
    ...
    n = size(data, 1);
    x = 1:n; 
    build_stacked_bar_chart(data, x, 'northeast', y_lim, strcat('Python', bar_titles_full{i}), strcat(image_base_path,bar_image_paths_full{i}), bar_width, '%', bar_ticklabels(i), {}, absolute);
end
...
n = size(data, 1);
x = 1:n;  
build_stacked_bar_chart(data, x, 'northeast', y_lim, 'Python Test Correctness', strcat(image_base_path,'correctness_bar.png'), bar_width, '%', bar_ticklabels, {}, absolute);
